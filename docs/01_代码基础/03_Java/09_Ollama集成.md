...
要点：
- `ProcessBuilder` 启动外部进程（这里启动 `cmd.exe` 间接驱动 ollama serve）
- `BufferedWriter` 写入子进程 stdin，`BufferedReader` 读取 stdout
- `ProcessBuilder.redirectErrorStream(true)` 把 stderr 合并到 stdout

## 示例 1：启动 Ollama 服务

```java
// 原 src/MyLearn/Ollama.java（已去除 package 声明）
...
## 示例 2：与 Ollama 进程交互（发送命令 + 读取响应）

```java
// 原 src/MyLearn/OllamaInteractor.java（已去除 package 声明）
